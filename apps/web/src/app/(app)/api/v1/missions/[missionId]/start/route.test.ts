...
const DB_FILE = `/tmp/forge-v1-start-route-${process.pid}.db`;
...
describe('POST /api/v1/missions/[missionId]/start', () => {
  ...
  it('409s when the mission is not in a startable state', async () => {
    const missionId = `msn_${randomUUID().replaceAll('-', '').slice(0, 12)}`;
    await insertMission(missionId, 'owner_1', { status: 'running' });

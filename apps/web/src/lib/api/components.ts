...
export const responseSchemas: Record<string, JsonSchema> = {
  ...
  'ledger.mission': envelope({ events: collection('LedgerEvent'), nextCursor: nullable(str) }),
  'ledger.task': envelope({ events: collection('LedgerEvent'), nextCursor: nullable(str) }),

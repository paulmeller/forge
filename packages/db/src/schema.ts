...
export const missions = sqliteTable(
  'missions',
  {
    ...
    /**
     * GitHub's `X-GitHub-Delivery` header — a GUID unique per webhook
     * delivery *attempt*, including retries and redeliveries of the same
     * event. dispatchFromGithub dedupes on this: a redelivered `@forge`
     * comment (or two concurrent invocations racing on the same delivery)
     * must resolve to the same Mission, not a second one. The unique index
     * below is what makes the race-safe — SQLite allows any number of NULL
     * rows through a unique index, so UI/API-created missions (which have no
     * GitHub delivery behind them) are unaffected. Null for those.
     */
  ...
  (t) => [uniqueIndex('missions_github_delivery_id_unique').on(t.githubDeliveryId)],
);
